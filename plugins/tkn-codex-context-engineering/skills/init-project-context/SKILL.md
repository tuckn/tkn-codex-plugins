...
- Create a new `~/.tkn/codex-context/state/<projectId>/working-context.md` with schema v2 portfolio metadata and sections, plus `sessions/`, `decisions/`, and `memos/`.
- Treat an existing working context without `schemaVersion` as legacy v1. Identity-only initialization may make v1 explicit but must not relabel its unchanged body as v2. Preserve existing v1 or v2 and refuse unsupported versions.
