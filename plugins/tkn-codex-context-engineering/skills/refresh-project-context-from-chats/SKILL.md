...
3. この Skill と同じ `skills/` directory にある次の3ファイルを完全に読む。
   ...
   3 artifact の新規作成と既存 file 更新では、各 Skill の `schemaVersion` と schema compatibility policy に従う。
   Working context の `Recent Decisions` と `Key Files` を更新する場合は、同 Skill の `project:/` と `state:/` logical reference policy に従う。
