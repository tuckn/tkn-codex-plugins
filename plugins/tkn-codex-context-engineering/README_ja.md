...
Project decision と project working context までの project-scoped stage は bundled Skills で実装済みです。
全 project の月次 chat review、reusable-context candidate の生成、登録済み project の portfolio
集約も実装済みです。一方、user-global decision への最終書き込みは今後の downstream stage
として想定しており、現時点では bundled write Skill として提供していません。
...
```text
fresh な project working contexts
  -> write-global-working-context
  -> 登録済み project の portfolio

reviewed project decisions + monthly chat reviews
  -> audit と project 横断 review
  -> global decision、Skill、automation の candidates
  -> human review と承認
...
Portfolio 集約と candidate review は実装済みですが、user-global decision への最終 write は現在の
bundled write surface の外にあります。
...
Session note は source に近い factual digest です。Chat より短くしながら、何を依頼され、何が変わり、
作業がどこで止まったかを理解するための事実を残します。

主に残すもの:

- ユーザーの request、clarification、correction
- 実際に行った action と報告・観測された result
- explicit decision と validation result
- last known work state と latest user direction
- 必要な場合だけ material evidence と source limitation
...
### 5. 登録済み Codex Project の現在状態を統合する

`write-global-working-context` は、各登録済み project の `working-context.md` を source input
として、明示的に設定した private destination の portfolio dashboard に統合します。例:

```text
<portfolio-context-root>/state/working-context.md
```

この Skill は private registry から project を解決し、machine または user 固有の path を
埋め込まず、write 時には private destination の明示指定を要求します。Project working context
だけを読み、raw chat、session note、decision body は読みません。

生成される global working context は、chat transcript や全 session note を直接要約して作るもの
ではありません。各 project ですでに蒸留された current truth を集約し、project 間の関係、
優先順位、blocker、次に着手すべき work を把握するための portfolio-level dashboard です。

想定する内容:

- Active、paused、blocked、archived projects
- Project ごとの目的、status、current focus、next step
- Project 間で宣言された dependency
- Review または maintenance が必要な stale project context

Duplicated work や project 横断の共通 pattern は semantic review が必要なため、この deterministic
writer では推測せず、review/materialization stage の candidate として扱います。
...
Bundled Skills は global decision candidate の抽出と review まで対応していますが、user-global
decision への最終 materialization は現在の bundled write surface の外にあります。外部または
将来の workflow では、明示的に設定した private destination を使用します。例:
...
| Layer | Primary artifact | 主な問い | 更新特性 | 現在の対応状況 |
|---|---|---|---|---|
| Source | Codex JSONL chat | 実際に何が話されたか | append-only / read-only | 実装済み |
| Session | `sessions/*.md` | この chat で何を行い、どう再開するか | thread ごとに更新 | 実装済み |
| Project decision | `decisions/DR-*.md` | 今後も守る判断は何か | durable、status 管理 | 実装済み |
| Project current state | `working-context.md` | この project で今何が真か | stale content を置換 | 実装済み |
| Global current state | user-global `state/working-context.md` | 登録済み project 全体の今はどうなっているか | project dashboards から集約 | 実装済み |
| Global knowledge | user-global `data/decisions/` 等 | project を超えて再利用すべきものは何か | reviewed promotion | candidate review は実装済み。最終書き込みは外部 |
| Insight / Materialization | monthly reviews、Skill candidates 等 | 何を改善、自動化、体系化すべきか | periodic review | 月次 source review は実装済み。materialization は follow-up が必要 |
...
新規 Project session note、decision record、working context は `schemaVersion: 2` を使用します。
Session v2 は source-near な factual digest、decision v2 は再利用可能な判断単位、working-context v2 は
portfolio metadata を持つ project control plane です。Reader は以前の artifact への対応を維持できますが、
writer Skill は current output contract だけを定義します。未対応 version の意味は推測せず拒否します。

| Artifact | v2 の役割 | 安定して抽出する signal |
|---|---|---|
| Session note | Source-near な factual digest | summary、key developments、last known state、evidence、source notes |
| Decision record | 再利用可能な判断単位 | rationale、applicability、`implementationStatus`、verification、materialization、supersession |
| Working context | Project control plane | lifecycle、health、priority、focus、blocker、next action、activity/review date、dependencies |

Current field、section、enum の canonical contract は、対応する writer Skill に置きます。
...
- Portfolio working-context への write では明示的な private destination を要求します。
- User-global decision への最終 write は bundled Skill として提供していません。

### Working-context の path reference

Schema v2 の Project `working-context.md` では、`Effective Decisions` と
`Key Files And Evidence` にある file・directory reference に次の logical root を使います。
...
- `write-global-working-context`:
  - 登録済み project の working context を、source reference 付き portfolio dashboard として、
    明示的に選択した private destination に集約します。
...
- `plan-context-rebuild-from-chats`:
  - Registry の全 project と名前付きの複数 Codex sessions source を照合し、既存 artifact の
    ...
    read-only の再構築 plan を作成します。Review後は、新しいshadow treeへschema v2 session note
    とstale working-context draftを生成し、decision candidateはreview gateの内側に保ちます。
