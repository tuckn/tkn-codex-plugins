...
Project session note、decision record、working context は `schemaVersion: 1` を使用します。
`schemaVersion` がない既存 artifact は legacy v1 として扱い、次に更新した時点で version を明示します。
未対応 version の構造を reader や writer が推測してはいけません。Field の意味、必須 metadata、本文
section、または downstream extraction contract に互換性のない変更を加える場合だけ version を上げます。
