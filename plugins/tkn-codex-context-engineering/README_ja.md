...
全 project の月次 chat review と reusable-context candidate の生成も実装済みです。一方、登録済み
...
各登録済み project の `working-context.md` を source input として、明示的に設定した private
destination の portfolio dashboard に統合することを想定します。例:

```text
<portfolio-context-root>/state/working-context.md
```

この portfolio 集約 stage は intended design の一部ですが、現時点では bundled write Skill として
提供していません。将来の実装では private registry から project を解決し、machine または user
固有の path を埋め込まず、private destination を明示的に指定する必要があります。
...
| Global current state | user-global `state/working-context.md` | 登録済み project 全体の今はどうなっているか | project dashboards から集約 | intended。bundled writer は未実装 |
