...
Session v2 は蒸留可能な evidence package、decision v2 は再利用可能な判断単位、working-context v2 は
portfolio metadata を持つ project control plane です。`schemaVersion` がない artifact は legacy v1
として扱います。Reader は v1 と v2 を読めますが、writer は v1 本文の意味を v2 構造へ移さず番号だけ
付け替えてはいけません。それ以外の version は未対応です。

| Artifact | v2 の役割 | 安定して抽出する signal |
|---|---|---|
| Session note | 蒸留可能な evidence package | confirmations、evidence、`DC-NN`、reusable learnings、open loops、handoff |
...
Field、section、enum、v1→v2 migration の canonical contract は、対応する writer Skill に置きます。
